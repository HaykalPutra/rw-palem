...
<section class="bg-gradient-to-br from-slate-50 to-blue-50/40 border-b border-slate-100">
  <div class="max-w-3xl mx-auto px-6 pt-14 pb-12 text-center reveal">
    ...
    <form method="GET" action="{{ route('berita') }}" class="mt-7 flex items-center gap-2 max-w-lg mx-auto">
      <input type="search" name="q" value="{{ $search }}" placeholder="Cari judul atau isi berita…"
      ...
      @if ($search !== '')
        <a href="{{ route('berita') }}" class="text-sm text-slate-500 hover:text-slate-700 px-2">Reset</a>
      @endif
    </form>

    @if ($search !== '')
      <p class="text-xs text-slate-500 mt-3">Menampilkan hasil untuk “{{ $search }}” ({{ $berita->total() }} ditemukan)</p>
    @endif
  </div>
</section>
...
<section class="max-w-6xl mx-auto px-6 py-12 pb-20 grid grid-cols-1 md:grid-cols-3 gap-10">
  <div class="md:col-span-2">
    @if ($headline)
      <a href="{{ route('berita.show', $headline) }}" class="block group">
        <div class="rounded-2xl overflow-hidden mb-5 relative shadow-md reveal-scale">
        ...
        <h2 class="text-xl font-extrabold text-slate-800 mb-2 leading-snug group-hover:text-blue-600 transition-colors">{{ $headline->title }}</h2>
        <p class="text-sm text-slate-500 mb-2 leading-relaxed">{{ $headline->excerpt ?: \Illuminate\Support\Str::limit(strip_tags($headline->content), 180) }}</p>
        <span class="text-sm font-semibold text-blue-600 group-hover:text-blue-800 transition-colors">Baca selengkapnya →</span>
      </a>
    @endif
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      @foreach ($items->slice($headline ? 1 : 0) as $item)
          <a href="{{ route('berita.show', $item) }}" class="group border border-slate-100 rounded-2xl overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 bg-white reveal-scale block">
          ...
          <div class="p-4">
            ...
            <h4 class="text-sm font-bold text-slate-800 mb-1.5 leading-snug group-hover:text-blue-600 transition-colors">{{ $item->title }}</h4>
      ...
      @if ($berita->isEmpty())
        <div class="sm:col-span-2 border border-dashed border-slate-200 rounded-2xl p-12 text-center">
          ...
          <div class="text-sm font-semibold text-slate-600 mb-1">{{ $search !== '' ? 'Berita tidak ditemukan' : 'Belum ada berita' }}</div>
          <p class="text-xs text-slate-400">{{ $search !== '' ? 'Coba kata kunci lain.' : 'Tambahkan dari halaman admin.' }}</p>
        </div>
      @endif
    </div>
